...
```bash
config_path: str, path to the folder with primary training/testing configs, relative to /opt/nvidia/bionemo
config_name: str, hydra config name to use that can be found under config_path
script_path: str, path to a folder with training/testing scripts to use, relative to /opt/nvidia/bionemo
...
```
The created partial training command in JET using those arguments is executed under `BIONEMO_HOME=/opt/nvidia/bionemo` in a BioNeMo's docker container
...
python internal/jet/run_jet_downstream_pipeline.py --model megamolbart --config_path examples/tests/conf --config_name megamolbart_test \
 ...
 --extra_overwrites "hydra.searchpath=[file:///opt/nvidia/bionemo/examples/molecule/megamolbart/conf] ++model.dwnstr_task_validation.enabled=False"
```
...
`--image gitlab-master.nvidia.com/clara-discovery/bionemo:dev-latest-devel` (default in the config)
  ...
  --script_path examples/molecule/megamolbart --variant pretrain --nodes 1 --gpus 1 --batch_size 8 --precision 16 \
   --extra_overwrites "trainer.max_steps=200 ++trainer.check_val_every_n_epoch=null ++model.data.dataset_path=/opt/nvidia/bionemo/examples/tests/test_data/molecule ++model.data.dataset.train=x000 ++model.data.dataset.val=x000 ++model.data.dataset.test=x000 ++model.dwnstr_task_validation.enabled=False"
  ```
...
Supply `--git_repo` (link of the git repo with the code related to the new model), `--git_branch` (the related branch name) and `--dockerfile` (path to the `Dockerfile` that is used to build a docker for testing, relative to `/opt/nvidia/bionemo`).
For instance, 
```bash
python internal/jet/run_jet_downstream_pipeline.py --git_repo "https://gitlab-master.nvidia.com/clara-discovery/bionemo.git" \
...
python examples/molecule/megamolbart/pretrain.py --config-path /opt/nvidia/bionemo/examples/tests/conf/ --config-name megamolbart_test \
trainer.num_nodes=1 trainer.devices=8 trainer.precision=32 model.micro_batch_size=32 \
++model.dwnstr_task_validation.enabled=False \
++exp_manager.create_wandb_logger=False ++exp_manager.create_tensorboard_logger=False ++exp_manager.exp_dir=/tmp/nemo_experiments/ ++exp_manager.create_checkpoint_callback=False ++exp_manager.resume_if_exists=False  hydra.searchpath=[file:///opt/nvidia/bionemo/examples/molecule/megamolbart/conf] \
