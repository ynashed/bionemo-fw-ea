...
ALLOWED_RECIPE_CONFIG_KWARGS_KEYS = ["config_path", "config_name", "script_path",
                                     "model", "variant", "extra_overwrites", "dllogger_warmup", "batch_size", "nodes",
                                     "gpus", "precision"]
...
def modify_jet_workloads_config(jet_workloads_repo_path: str, docker_image: Optional[str] = None,
    ...
    assert os.path.isdir(jet_workloads_repo_path), f"JET Workloads Repository does not exists: " \
                                                   f"{jet_workloads_repo_path}"

    if docker_image:
        _modify_jet_workloads_config(config_type='build',
    ...
    if git_repo and git_branch and dockerfile:
        _modify_jet_workloads_config(config_type='build',
                                     config_name=CONFIG_BUILD_NAME_DOCKER_IMAGE,
                                     jet_workloads_repo_path=jet_workloads_repo_path,
                                     config_kwargs={"git_repo": git_repo, "git_branch": git_branch,
                                                    "dockerfile": dockerfile})

    config_kwargs = {"config_path": config_path, "config_name": config_name,
                     "script_path": script_path, "model": model, "variant": variant,
                     "extra_overwrites": extra_overwrites, "dllogger_warmup": dllogger_warmup,
                     "batch_size": batch_size, "nodes": nodes, "gpus": gpus, "precision": precision}

    if any([v is not None for _, v in config_kwargs.items()]):

        _modify_jet_workloads_config(config_type='recipe', config_name=CONFIG_NAME_RECIPE_PERF,
                                     jet_workloads_repo_path=jet_workloads_repo_path, config_kwargs=config_kwargs)


def _modify_jet_workloads_config(config_type: str,
                                 config_name: str,
                                 jet_workloads_repo_path: str = None,
...
def _modify_jet_workloads_recipe_config(workloads_config_path: str,
                                        config_path: Optional[str] = None, config_name: Optional[str] = None,
                                        script_path: Optional[str] = None, variant: Optional[str] = None,
                                        model: Optional[str] = None, extra_overwrites: Optional[str] = None,
                                        dllogger_warmup: Optional[int] = None,
...
def _modify_jet_workloads_build_config(workloads_config_path: str, docker_image: Optional[str] = None,
                                       git_repo: Optional[str] = None, git_branch: Optional[str] = None,
                                       dockerfile: Optional[str] = None) -> None:
