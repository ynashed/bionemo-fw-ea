...
python internal/jet/run_jet_downstream_pipeline.py --jet_ref_eph ${JET_REF_EPH}  --jet_repo_path ${JET_WORKLOADS_PATH} \
  ...
  --precision 16 32 --extra_overwrites "trainer.max_steps=200 ++trainer.check_val_every_n_epoch=null ++model.data.dataset_path=/opt/nvidia/bionemo/examples/tests/test_data/molecule ++model.data.dataset.train=x000 ++model.data.dataset.val=x000 ++model.data.dataset.test=x000 ++model.dwnstr_task_validation.enabled=False"
