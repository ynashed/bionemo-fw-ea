...
jet_logs_table=$(jet -c -tf plain -th logs query -c "obj_workload.s_key" -c s_id --eq obj_ci.l_pipeline_id "${jet_pipeline_id}" --eq obj_workload.s_type "recipe")

for jet_log_id in ${jet_logs_table}; do
  if [[ $jet_log_id = recipe/* ]]
  then
    echo "==================================================================================================================="
