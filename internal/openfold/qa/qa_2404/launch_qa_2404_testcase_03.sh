...
#
# title: launch_qa_2404_testcase_03.sh
# usage:
#   sbatch path-to-script/launch_qa_2404_testcase_03.sh
# notes:
...
# (0) preamble
MESSAGE_TEMPLATE='********launch_qa_2404_testcase_03.sh: %s\n'
...
OUTPUT_DIR=/lustre/fsw/portfolios/convai/users/broland/qa/qa_202404/testcase_03_${DATETIME_SCRIPT_START}
...
srun --mpi=pmix \
  ...
  bash -c "trap : SIGTERM ; set -x; set -e; echo 'launch_qa_2404_testcase_03.sh - before date' &&
  echo "'date=$(date +'%Y%m%dT%H%M')'" &&
  export HYDRA_FULL_ERROR=1 &&
  cd /workspace/bionemo &&
  echo 'launch_qa_2404_testcase_03.sh - before install_third_party.sh' &&  
  ./examples/protein/openfold/scripts/install_third_party.sh &&
  echo 'launch_qa_2404_testcase_03.sh - after install_third_party.sh' &&  
  echo 'launch_qa_2404_testcase_03.sh - before download_artifacts.py' &&  
  python download_artifacts.py --source pbss --download_dir models --models openfold_initial_training_inhouse &&
  echo 'launch_qa_2404_testcase_03.sh - after download_artifacts.py' &&
  echo 'launch_qa_2404_testcase_03.sh - before train.py' &&
  ...
  echo "'date=$(date +'%Y%m%dT%H%M')'" &&
  echo 'launch_qa_2404_testcase_03.sh - after everything'"
