...
export AWS_SECRET_ACCESS_KEY=cc3e6b4c86196d66bb356f985b6517a1

# (0) preamble
MESSAGE_TEMPLATE='********launch_qa_testcase_03.sh: %s\n'
...
srun --mpi=pmix \
  ...
  bash -c "trap : SIGTERM ; set -x; set -e; echo 'launch_qa_testcase_03.sh - before date' &&
  ...
  echo "'date=$(date +'%Y%m%dT%H%M')'" &&
  echo 'launch_qa_testcase_03.sh - after everything'"
