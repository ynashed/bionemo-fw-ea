...
#
# title: launch_qa_testcase_10.sh
# description: initial training, single node, fused adam-swa
#
# usage:
#   (1) update the parameter IMAGE_NAME in this file
#   (2) sbatch path-to-script/launch_qa_testcase_10.sh
#
# notes:
...
#   (b) train runs with fused adam-swa
...
# (0) preamble
MESSAGE_TEMPLATE='********launch_qa_testcase_10.sh: %s\n'
...
srun --mpi=pmix \
  ...
  bash -c "trap : SIGTERM ; set -x; set -e; echo 'launch_qa_testcase_10.sh - before date' &&
  ...
  echo "'date=$(date +'%Y%m%dT%H%M')'" &&
  echo 'launch_qa_testcase_10.sh - after everything'"
