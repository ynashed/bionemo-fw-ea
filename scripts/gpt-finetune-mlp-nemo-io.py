...
from nemo.lightning.megatron_parallel import DataT, MaskedTokenLossReduction
...
class ExtendedModel(pl.LightningModule):  # io.IOMixin, io.ConnectorMixin, fn.FNMixin
    def __init__(self, tokenizer, ckpt_path):
        super().__init__()
        self.tokenizer = tokenizer
        self.ckpt_path = ckpt_path
        self.optim = MegatronOptimizerModule(
            config=OptimizerConfig(lr=1e-4, use_distributed_optimizer=True)
        )  # TODO(@georgea) allow optim to be passed in?
        self.optim.connect(self)  # This will bind the `configure_optimizers` method
        # TODO infer from ckpt file?
        self.config = llm.GPTConfig(
            num_layers=4,
            hidden_size=256,
            ffn_hidden_size=512,
    ...
    def configure_model(self) -> None:
        self.module = llm.GPTModel(self.config, tokenizer=self.tokenizer)
        if self.trainer:
            fabric = self.trainer.to_fabric()
            self.module = fabric.load_model(self.ckpt_path)
...
if __name__ == "__main__":
    ...
    ckpt_path = "/workspace/bionemo2/lightning_logs/version_10/checkpoints/epoch=0-step=9"
    log = NeMoLogger()
    ...
    model = ExtendedModel(tokenizer=data_module.tokenizer, ckpt_path=ckpt_path)
