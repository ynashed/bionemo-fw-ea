...
class Unbound_Bound_Data(Dataset):
    ...
    def __init__(self, cfg, if_swap=True, reload_mode='train'):
        ...
        self.ligand_graph_filename = os.path.join(
            cache_path, 'ligand_graph_' + frac_str + reload_mode + '.bin')
        self.receptor_graph_filename = os.path.join(
        ...
        self.bound_ligand_repres_nodes_loc_array_list = label[
            'bound_ligand_repres_nodes_loc_array_list']
        self.bound_receptor_repres_nodes_loc_array_list = label[
    ...
    def __getitem__(self, idx):
        ...
        receptor_graph_list, _ = load_graphs(
            self.receptor_graph_filename, [idx])
        ...
        if swap:  # Just as a sanity check, but our model anyway is invariant to such swapping.
            bound_ligand_repres_nodes_loc_array = self.bound_receptor_repres_nodes_loc_array_list[
                idx]
            bound_receptor_repres_nodes_loc_array = self.bound_ligand_repres_nodes_loc_array_list[
                idx]
            ligand_graph = receptor_graph_list[0]
            receptor_graph = ligand_graph_list[0]
        else:
            bound_ligand_repres_nodes_loc_array = self.bound_ligand_repres_nodes_loc_array_list[
                idx]
            bound_receptor_repres_nodes_loc_array = self.bound_receptor_repres_nodes_loc_array_list[
                idx]
        ...
        if len(self.rot_T) != 0:
            ...
            rot_T, rot_b = UniformRotation_Translation(
                translation_interval=self.cfg['translation_interval'])
        ...
        pocket_coors_ligand = (
            rot_T @ (pocket_coors_ligand - mean_to_remove).T).T + rot_b
        ligand_new_loc = (
            rot_T @ (ligand_original_loc - mean_to_remove).T).T + rot_b

        ligand_graph.ndata['new_x'] = torch.from_numpy(
            ligand_new_loc.astype(np.float32))
...
def nemo_get_dataset(args, reload):
    ...
    if reload not in ['train', 'val', 'test']:
        raise ValueError(
            f"reload(={reload}) is not valid, choose from train, val, and test!")
