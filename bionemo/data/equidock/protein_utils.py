...
from bionemo.data.equidock.protein_featurizers import residue_type_one_hot_dips_not_one_hot,  residue_type_one_hot_dips
...
def get_coords(filename, all_atoms=True):
    ...
    if not all_atoms:
        ...
        coords = df[['x_coord', 'y_coord', 'z_coord']
...
def rigid_transform_Kabsch_3D_torch(A, B):
    ...
    with autocast(enabled=False):
        ...
        if torch.linalg.det(R) < 0:
            SS = torch.diag(torch.as_tensor(
                [1., 1., -1.], device=A.device, dtype=A.dtype))
            R = torch.matmul(torch.matmul(Vt.T, SS), U.T)
        if torch.abs(torch.linalg.det(R) - 1.).item() > 2e-3:
            logging.info(
...
def distance_list_featurizer(dist_list):
    ...
    processed_features['he'] = torch.from_numpy(
        transformed_dist.astype(np.float32))
    return processed_features


def residue_list_featurizer_dips_one_hot(predic):
    residue_list = [term[1]['resname'].iloc[0] for term in predic]
    feature_list = [residue_type_one_hot_dips(
        residue) for residue in residue_list]
    feature_list = np.stack(feature_list)
    processed_features = dict()
    processed_features['res_feat'] = torch.from_numpy(
        feature_list.astype(np.float32))
    return processed_features


def residue_list_featurizer_dips_NOT_one_hot(predic):
    residue_list = [term[1]['resname'].iloc[0] for term in predic]
    feature_list = [[residue_type_one_hot_dips_not_one_hot(
        residue)] for residue in residue_list]
    feature_list = np.array(feature_list)
    processed_features = dict()
    processed_features['res_feat'] = torch.from_numpy(
        feature_list.astype(np.float32))  # (N_res, 1)
    return processed_features


def preprocess_unbound_bound_dips(file_loc, graph_nodes, pos_cutoff=8.0, inference=False):

    bound_ligand_residues, bound_receptor_residues = get_residues_DIPS(
        file_loc)
   ...
   ####################
    ...
    bound_receptor_repres_nodes_loc_array = get_alphaC_loc_array(
        bound_predic_receptor_clean_list)
    bound_ligand_repres_nodes_loc_array = get_alphaC_loc_array(
        bound_predic_ligand_clean_list)
    ...
    if not inference:
        ...
        if active_ligand.size <= 3:  # We need: active_ligand.size > 0 '
            ...
            assert np.max(np.linalg.norm(ligand_pocket_coors -
                          receptor_pocket_coors, axis=1)) <= pos_cutoff
            pocket_coors = 0.5 * (ligand_pocket_coors + receptor_pocket_coors)
            print('Num pocket nodes = ', len(active_ligand), ' total nodes = ',
...
def protein_to_graph_unbound_bound_residuesonly(unbound_ligand_predic,
    ...
    def l_or_r_extract_3d_coord_and_n_u_v_vecs(l_or_r_predic):
        ...
        for residue in l_or_r_predic:
            df = residue[1]
            coord = df[['x', 'y', 'z']].to_numpy().astype(
                np.float32)  # (N_atoms, 3)
            ...
            if Natom.shape[0] != 1 or alphaCatom.shape[0] != 1 or Catom.shape[0] != 1:
                print(df.iloc[0, :])
                raise ValueError(
                    "protein utils protein_to_graph_unbound_bound, no N/CA/C exists")

            N_loc = Natom[['x', 'y', 'z']].to_numpy(
            ).squeeze().astype(np.float32)
            alphaC_loc = alphaCatom[['x', 'y', 'z']
                                    ].to_numpy().squeeze().astype(np.float32)
            C_loc = Catom[['x', 'y', 'z']].to_numpy(
            ).squeeze().astype(np.float32)
            ...
            assert (math.fabs(LA.norm(v_i) - 1.) <
                    1e-5), "protein utils protein_to_graph_dips, v_i norm larger than 1"
        ...
        l_or_r_residue_representatives_loc_feat = np.stack(
            l_or_r_residue_representatives_loc_list, axis=0)  # (N_res, 3)
    ...
    def compute_dig_kNN_graph(l_or_r_num_residues,
        ...
        l_or_r_distance = np.full(
            (l_or_r_num_residues, l_or_r_num_residues), np.inf)
        ...
        for i in range(l_or_r_num_residues):
            ...
            if len(valid_src) > max_neighbor:
                valid_src = list(np.argsort(l_or_r_distance[i, :]))[
                    0: max_neighbor]
        ...
        l_or_r_protein_graph.add_edges(torch.IntTensor(
            l_or_r_src_list), torch.IntTensor(l_or_r_dst_list))

        if one_hot:
            l_or_r_protein_graph.ndata.update(
                residue_list_featurizer_dips_one_hot(unbound_l_or_r_predic))
        else:
            l_or_r_protein_graph.ndata.update(
                residue_list_featurizer_dips_NOT_one_hot(unbound_l_or_r_predic))

        l_or_r_protein_graph.edata.update(
            distance_list_featurizer(l_or_r_dist_list))
        ...
        for i in range(len(l_or_r_dist_list)):
            ...
            q_ij = np.matmul(
                basis_matrix, l_or_r_n_i_feat[src, :])  # shape (3,)
            k_ij = np.matmul(basis_matrix, l_or_r_u_i_feat[src, :])
            t_ij = np.matmul(basis_matrix, l_or_r_v_i_feat[src, :])
            s_ij = np.concatenate((p_ij, q_ij, k_ij, t_ij),
                                  axis=0)   # shape (12,)
            l_or_r_edge_feat_ori_list.append(s_ij)
        l_or_r_edge_feat_ori_feat = np.stack(
            l_or_r_edge_feat_ori_list, axis=0)  # shape (num_edges, 4, 3)
        l_or_r_edge_feat_ori_feat = torch.from_numpy(
            l_or_r_edge_feat_ori_feat.astype(np.float32))
        l_or_r_protein_graph.edata['he'] = torch.cat(
            (l_or_r_protein_graph.edata['he'], l_or_r_edge_feat_ori_feat), axis=1)  # (num_edges, 17)
        ...
        l_or_r_protein_graph.ndata['mu_r_norm'] = torch.from_numpy(
