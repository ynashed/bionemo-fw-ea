...
def megamolbart_build_train_valid_test_datasets(cfg: DictConfig, train_valid_test_num_samples: Dict[str, Optional[int]]) \
        -> List[Dataset]:
    """
    ...
    assert all([split in ['train', 'val', 'test'] for split in train_valid_test_num_samples.keys()]), \
    ...
    for split in train_valid_test_num_samples.keys():
        ...
        if num_samples is None or num_samples > 0:
            ...
            assert ds_name is not None, f'Config "cfg" should contain field "cfg.dataset.{split}" with name or list of ' \
            ...
            dataset = build_typed_dataset(dataset_paths=filepath, data_impl=data_impl, use_upsampling=use_upsampling,
                                          cfg=cfg, num_samples=num_samples)
        else:
            dataset = None
...
def megamolbart_retro_build_train_valid_test_datasets(cfg: OmegaConf,
                                                      train_valid_test_num_samples: Dict[str, Optional[int]]) \
    ...
    assert all([split in data_preprocessor.splits for split in train_valid_test_num_samples.keys()]), \
        'Incorrect key in train_valid_test_num_samples!'
