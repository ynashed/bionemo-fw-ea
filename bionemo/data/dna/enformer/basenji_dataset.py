...
class WebBasenji(wds.pipeline.DataPipeline, wds.compat.FluidInterface):
    ...
    def __init__(self, dataset_path: str, subset:str, organism: str, shuffle: bool = False, context_length: int = 196_608, batch_size:int = 1, repeat=False):
        super().__init__()
...
class WebCombinedBasenji(torch.utils.data.IterableDataset): 
  def __init__(self, dataset_path: str, context_length: int = 196_608, batch_size: int = 1):
    ...
    self.human_dataset = WebBasenji(dataset_path, 'train', 'human', repeat=True, context_length=context_length, batch_size=batch_size)
    self.mouse_dataset = WebBasenji(dataset_path, 'train', 'mouse', repeat=True, context_length=context_length, batch_size=batch_size)
