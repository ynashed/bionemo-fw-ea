...
def convert_ob_mol_to_rd_mol(ob_mol, struct=None):
    ...
    for d, bond in nonsingles:
        ...
        if calc_valence(a1) > pt.GetDefaultValence(a1.GetAtomicNum()) or calc_valence(a2) > pt.GetDefaultValence(
            a2.GetAtomicNum()
        ):
...
def fixup(atoms, mol, indicators):
    ...
    for i, atom in enumerate(atoms):
        ...
        # Leaving commented to preserve potential logic
...
def reconstruct_from_generated(xyz, atomic_nums, aromatic=None, basic_mode=True):
    """
    utilize data.ligand_pos, data.ligand_element, data.ligand_atom_feature_full to reconstruct mol
    """
    if basic_mode:
        indicators = None
    else:
...
def get_bond_order_batch(atoms1, atoms2, distances):
    ...
    bonds1 = torch.tensor(crossdocked_bonds1, device=atoms1.device)
    bonds2 = torch.tensor(crossdocked_bonds2, device=atoms1.device)
    bonds3 = torch.tensor(crossdocked_bonds3, device=atoms1.device)
...
for a in atypes:
    value = []
    for b in atypes:
        value.append(bonds1[a][b])
    crossdocked_bonds1.append(value)

crossdocked_bonds2 = []
for a in atypes:
    value = []
    for b in atypes:
        if a not in bonds2 or b not in bonds2[a]:
            value.append(0.0)
        else:
            value.append(bonds2[a][b])
    crossdocked_bonds2.append(value)

crossdocked_bonds3 = []
for a in atypes:
    value = []
    for b in atypes:
        if a not in bonds3 or b not in bonds3[a]:
            value.append(0.0)
        else:
            value.append(bonds3[a][b])
    crossdocked_bonds3.append(value)
