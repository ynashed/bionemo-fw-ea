...
from bionemo.data.diffdock.conformer_matching import get_torsion_angles, optimize_rotatable_bonds
from bionemo.model.molecule.diffdock.utils.torsion import get_transformation_mask
...
def get_calpha_graph(
    rec, c_alpha_coords, n_coords, c_coords, complex_graph, cutoff=20, max_neighbor=None, lm_embeddings=None
):
    n_rel_pos = n_coords - c_alpha_coords
    c_rel_pos = c_coords - c_alpha_coords
    num_residues = len(c_alpha_coords)
    if num_residues <= 1:
        raise ValueError("rec contains only 1 residue!")
    ...
    for i in range(num_residues):
        ...
        dst.remove(i)
        if max_neighbor is not None and len(dst) > max_neighbor:
...
def get_fullrec_graph(
    ...
    for i in range(num_residues):
        ...
        dst.remove(i)
        if c_alpha_max_neighbors is not None and len(dst) > c_alpha_max_neighbors:
    ...
    atoms_edge_index = radius_graph(
        atom_coords, atom_cutoff, max_num_neighbors=atom_max_neighbors if atom_max_neighbors else 1000
    )
