    ...
    def __getitem__(self, idx):
        ...
        if isinstance(embeddings, str):
            ...
            embeddings = torch.cat([
                embeddings, 
                torch.zeros((self.data.max_length - seq_len), emb_dim)
                ])
            ...
            with open(fname, "r") as f:
                for line_idx, line in tqdm(enumerate(f)):
                    ...
                    if line_idx == 0:
                        ...
                        if columns_in_header(self.label_names, header) and \
                            columns_in_header(self.mask_col, header, allow_none=True) and \
                            columns_in_header([self.sequence_col], header):
    ...
    def _tokenize_labels(tokenizers, labels_str, labels_sizes):
        ...
        for strings in (labels_str):
            ...
            for i in range(len(strings)):
                ...
                cur_labels.append(
                    torch.tensor(np.array(cur_one_hot))
                    )
