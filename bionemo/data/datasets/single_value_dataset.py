...
class SingleValueDataset(Dataset):
    def __init__(self, datafiles, max_seq_length, emb_batch_size=None, 
                 ...
                 target_column: str='y', task: str="regression", shuffle=False):
        dfs = []
        ...
        if len(self.input_seq) == 0:
            raise ValueError("All sequences in the dataset are longer than the maximum allowed sequence length {}".format(self.max_seq_length - 2))
        self.embeddings = []
        self.masks = []
...
class SingleValueDataModule(BioNeMoDataModule):
    ...
    def _create_dataset(self, split, files, shuffle=False):
        ...
        if self.tokenizer is not None:
            ...
            dataset.labels = get_data._tokenize_labels([self.tokenizer], dataset.labels.reshape(1, 1, -1), [self.cfg.num_classes])[0][0].argmax(axis=1) 
        return dataset
