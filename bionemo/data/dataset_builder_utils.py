...
_DATA_IMPL_TYPE_CLS = {_CSV_FIELDS_MMAP_TYPE: CSVFieldsMemmapDataset,
                       _CSV_MMAP_TYPE: CSVMemMapDataset,
                       _FASTA_FIELDS_MMAP_TYPE: FASTAFieldsMemmapDataset
                       }


def build_typed_dataset(dataset_paths: Union[str, List[str]], data_impl: str, use_upsampling: bool, cfg: OmegaConf,
    ...
    assert data_impl in _DATA_IMPL_TYPE_CLS.keys(), \
        f'Argument data_impl must be set to: {", ".join(_DATA_IMPL_TYPE_CLS.keys())}'
    dataset_cls = _DATA_IMPL_TYPE_CLS[data_impl]

    assert 'data_impl_kwargs' in cfg, f"Config 'cfg' should contain 'data_impl_kwargs.{data_impl}' key being " \
    ...
    if isinstance(dataset_paths, list):
        dataset_paths: List[str] = [path for dataset_path in dataset_paths for
                                    path in expand_dataset_paths(dataset_path, ext=ext)]
