...
def sampling(
    ...
    for t_idx in range(denoising_inference_steps):
        ...
        for complex_graph_batch in data_loader:
            b = complex_graph_batch.num_graphs
            complex_graph_batch = complex_graph_batch.to(device)

            tr_sigma, rot_sigma, tor_sigma = t_to_sigma(t_tr, t_rot, t_tor)
            set_time(complex_graph_batch, t_tr, t_rot, t_tor, b, model_cfg.all_atoms, device)

            with torch.no_grad():
                tr_score, rot_score, tor_score = model(complex_graph_batch)

            tr_g = tr_sigma * torch.sqrt(
                torch.tensor(2 * np.log(model_cfg.diffusion.tr_sigma_max / model_cfg.diffusion.tr_sigma_min))
            )
            rot_g = (
                2
                * rot_sigma
                * torch.sqrt(
                    torch.tensor(np.log(model_cfg.diffusion.rot_sigma_max / model_cfg.diffusion.rot_sigma_min))
                )
            )
            ...
            if ode:
                ...
                tr_z = (
                    torch.zeros((b, 3))
                    if no_random or (no_final_step_noise and t_idx == denoising_inference_steps - 1)
            ...
            if not model_cfg.diffusion.no_torsion:
                tor_g = tor_sigma * torch.sqrt(
                    torch.tensor(2 * np.log(model_cfg.diffusion.tor_sigma_max / model_cfg.diffusion.tor_sigma_min))
                )
                if ode:
