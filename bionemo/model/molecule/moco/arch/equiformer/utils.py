...
#         X_v, R_v, T_v = X_v.clone(), R_v.clone(), T_v.clone()
...
# def compute_FAPE_uv(X_uv, mask_atom_uv, X_uv_pred, eps=1e-4, d_max=10.0, Z=10.0, weights=None, return_count=False):
...
#     denom[denom == 0] = 1.0
#     pos_mu = torch.sum(
#         pos * pos_mask[:, :, None], dim=1, keepdim=True) / denom[:, :, None]
#     ref_mu = torch.sum(
#         ref * pos_mask[:, :, None], dim=1, keepdim=True) / denom[:, :, None]
#     pos_c = pos - pos_mu
#     ref_c = ref - ref_mu
...
#     qslerp = (q0 * torch.sin((1 - t) * theta) + q1 *
#               torch.sin(t * theta)) / torch.sin(theta)
#     torch._assert(
#         torch.all(torch.logical_or(
#             theta == 0, torch.remainder(theta, torch.pi / 2) != 0)),
#         "theta which is multiple of pi/2 needs to be handled in quaternion_slerp2",
#     )
#     torch._assert(
#         torch.all(torch.logical_or(
#             theta == 0, torch.remainder(theta, torch.pi) != 0)),
...
# def compute_struct_loss(X, data, eps=1e-6, return_full=False, bond_tol_scale=1.0, apply_mask=False):
#     """
#     X [natoms, 3]
...
#     # bond
#     l_pred = ((X[dst_bonds_i1] - X[dst_bonds_i2]
#                ).square().sum(-1) + eps).sqrt()
#     loss_bond = (dst_bonds_l - l_pred).abs() - bond_tol_scale * dst_bonds_tol
#     mask_bond = dst_bonds_mask  # mask unambiguous
#     # mask missing atoms
#     if apply_mask:
#         mask_bond = mask_bond * \
#             dst_atom_mask[dst_bonds_i1] * dst_atom_mask[dst_bonds_i2]
...
#     if apply_mask:
#         mask_angle = dst_atom_mask[dst_angles_i1] * \
#             dst_atom_mask[dst_angles_i2] * dst_atom_mask[dst_angles_i3]
...
#         T, R = get_euclidean_kabsch(x2_.reshape(
#             1, -1, 3), x1_.reshape(1, -1, 3), mask_.reshape(1, -1))
#         if i < niter - 1:
...
#     rmsd = ((x2[isubset] - x1_shifted[isubset]
#              ).square().sum() / isubset.sum()).sqrt()
