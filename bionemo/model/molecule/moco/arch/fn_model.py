...
class XEGNNK(MessagePassing):
    ...
    def __init__(
        ...
        self.phi_message = MLP(self.message_input_size, invariant_edge_feat_dim, invariant_edge_feat_dim)
        ...
        if self.use_cross_product:
            self.phi_x_cross = MLP(invariant_edge_feat_dim, invariant_edge_feat_dim, n_vector_features)
...
class DiTeBlock(nn.Module):
    ...
    def forward(
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(t_emb_h)[batch].chunk(
            6, dim=1
        )
        (
            edge_shift_msa,
