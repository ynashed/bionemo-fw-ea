...
class MegalodonDotFN(nn.Module):
    ...
    def forward(self, batch, X, H, E_idx, E, continuous_time, discrete_time):
        # torch.max(batch) + 1
        # import ipdb; ipdb.set_trace()
        ...
        x_te = self.time_embedding(continuous_time)
        h_te = self.time_embedding(discrete_time)
        # te = self.time_embedding(t)
        te_h = h_te[batch]
        # edge_batch, counts = torch.unique(batch, return_counts=True)
        # edge_batch = torch.repeat_interleave(edge_batch, counts * (counts - 1))  #
        edge_batch = batch[E_idx[0]]
        te_e = h_te[edge_batch]
        te_x = x_te[edge_batch]
        edge_attr = E
        ...
        for layer_index in range(len(self.dit_layers)):
            ...
            pos = self.egnn_layers[layer_index](batch, pos, H, E_idx, edge_attr, te_x)  #! TODO at time here
