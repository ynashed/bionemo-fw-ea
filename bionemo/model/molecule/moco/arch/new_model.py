...
class DiTeBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs,
        ...
        feature_embedder_size_output = hidden_size
        if self.input_edges:
            feature_embedder_size = 2 * hidden_size + edge_feature_size + dist_size
            ...
            feature_embedder_size = 2 * hidden_size + dist_size
            self.adaLN_edge_modulation = None

        if self.output_edges:
            feature_embedder_size_output += edge_feature_size
        # import ipdb; ipdb.set_trace()
        self.feature_embedder = MLP(feature_embedder_size, feature_embedder_size_output, feature_embedder_size_output)
    ...
    def forward(
        ...
        if self.output_edges:
            messages, edge_messages = messages[:, : -self.edge_feature_size], messages[:, -self.edge_feature_size :]
        ...
        if self.output_edges:
            ...
            # import ipdb; ipdb.set_trace()
            edge = edge_messages + edge_gate_msa * self.lin_edge0((y[src] + y[tgt]))
