...
from bionemo.model.molecule.moco.models.multitime_denoising_models import ModelBuilder
...
class MultiTimeGraph3DInterpolantModel(pl.LightningModule):
    def __init__(
        self,
        ...
        super(MultiTimeGraph3DInterpolantModel, self).__init__()
    ...
    def sample_time(self, batch):
        ...
        continuous_time = self.interpolants['x'].sample_time(
        ...
        discrete_time = self.interpolants['h'].sample_time(
            num_samples=batch_size,
            device=batch.x.device,
            method=self.interpolant_params.sample_time_method,
            mean=self.interpolant_params.sample_time_mean,
        ...
        return {"continuous": continuous_time, "discrete": discrete_time}
    ...
    def interpolate(self, batch, times):
        # for interpolant_idx, interp_param in enumerate(self.interpolant_params.variables):
        for index, interp_param in self.interpolant_param_variables.items():
    ...
    def calculate_loss(self, batch, out, times, stage="train"):
    ...
    def multi_time(self, times):
        eps = torch.rand(1).item()
        if eps < 0.5:
            times["discrete"] = 0 * times["discrete"]
            if self.interpolants['h'].time_type == "continuous":
                times["discrete"] += 1
            else:
    ...
    def conditional_sample(
        ...
        conditional_time = torch.tensor([timesteps - 1] * num_samples).to(self.device)
        for idx in tqdm(list(range(len(DT))), total=len(DT)):
            if early_stop and idx >= early_stop:
                break
            t = timeline[idx]
            dt = DT[idx]
            model_time = torch.tensor([t] * num_samples).to(self.device)
            time = {"continuous": model_time, "discrete": conditional_time}
            data = self.one_hot(data)
            data = self.aggregate_discrete_variables(data)
            data["batch"] = batch_index
            data["edge_index"] = edge_index
