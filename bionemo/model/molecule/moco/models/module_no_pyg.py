...
class NoPygGraph3DInterpolantModel(pl.LightningModule):
    def __init__(
        self,
        ...
        super(NoPygGraph3DInterpolantModel, self).__init__()
    ...
    def sample_time(self, batch):
        batch_size = batch['node_mask'].size(0)  # int(batch.batch.max()) + 1
    ...
    def pre_format_molecules(self, batch, batch_size):
        ...
        for index, interp_param in self.interpolant_param_variables.items():
            ...
            #     batch['edge_index'] = edge_index_global
                ...
                batch['charges'] = batch['charges'] + batch['node_mask'] * interp_param.offset
        return batch
        ...
        for index, interp_param in self.interpolant_param_variables.items():
            ...
            if interpolant is None:
                ...
                if interp_param.variable_name == "edge_attr":
                    _, batch[f"{interp_param.variable_name}_t"], _ = interpolant.interpolate_edges_nopyg(
                        batch["edge_mask"], batch[f"{interp_param.variable_name}"], time
                    )
                else:
                    _, batch[f"{interp_param.variable_name}_t"], _ = interpolant.interpolate_nopyg(
                        batch["node_mask"], batch[f"{interp_param.variable_name}"], time
                    )
        ...
        for idx in tqdm(list(range(len(DT))), total=len(DT)):
            ...
            for key, interpolant in self.interpolants.items():
                ...
                if "edge" in key:
                    ...
                    data[f"{key}_t"] = interpolant.step_nopyg(
                        xt=data[f"{key}_t"],
                        x_hat=out[f"{key}_hat"],
                        x0=prior[key],
                        mask=mask,
