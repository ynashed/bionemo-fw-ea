...
class MegaMolBARTInference(BaseEncoderDecoderInference):
    ...
    def sample(self,
        ...
        assert sampling_method in default_sampling_kwarg.keys(),\
        ...
        if sampling_method == 'greedy-perturbate':
            samples = self.hiddens_to_seq(perturbed_hiddens, sample_masks,
                                          sampling_method="greedy-search", sampling_kwargs={})

        elif sampling_method == 'topkp-perturbate':
            samples = self.hiddens_to_seq(perturbed_hiddens, sample_masks,
                                          sampling_method="topkp-sampling", sampling_kwargs=sampling_kwarg)

        elif sampling_method == 'beam-search-perturbate':
            if num_samples is not None:
                sampling_kwarg['beam_size'] = num_samples
            samples = self.hiddens_to_seq(perturbed_hiddens, sample_masks,
                                          sampling_method="beam-search", sampling_kwargs=sampling_kwarg)
