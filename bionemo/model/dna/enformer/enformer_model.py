...
from bionemo.data.dna.enformer.basenji_dataset import (WebBasenji,
                                                       WebCombinedBasenji)
...
class Enformer(ModelPT):
    ...
    def __init__(self, cfg , trainer, *args, **kwarg):
        ...
        filter_list = exponential_linspace_int(half_dim, cfg.dim, 
                                               num = (cfg.num_downsamples - 1), 
                                               divisible_by = cfg.dim_divisible_by)
    ...
    def setup_training_data(self, train_data_cfg: Union[omegaconf.DictConfig, Dict]):
        ...
        if ('human' in organisms) and ('mouse' in organisms):
            train_ds = WebCombinedBasenji(dataset_path=train_data_cfg.dataset_path, 
                                          batch_size=train_data_cfg.batch_size, 
                                          context_length=self.cfg.context_length)
        else:
            assert 'human' in organisms
            train_ds = WebBasenji(dataset_path=train_data_cfg.dataset_path, 
                                  subset='train', 
                                  organism='human', 
                                  batch_size=train_data_cfg.batch_size, 
                                  context_length=self.cfg.context_length, 
    ...
    def _single_infer_dataloader(self, cfg: Union[omegaconf.DictConfig, Dict], split: str):
        def dl_organism(organism: str):
            organism_ds = WebBasenji(dataset_path=cfg['dataset_path'], 
                                         subset=split,
                                         organism=organism, 
    ...
    def validation_epoch_end(self, outputs):
        for organism in self.cfg.metrics.organisms.keys():
            ...
            self.log(f'rpearson_{organism}', metric.compute().mean(), sync_dist=False ,on_step=False, on_epoch=True, logger=True, prog_bar=True)
            metric.reset()
