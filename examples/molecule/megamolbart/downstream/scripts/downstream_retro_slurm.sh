...
# Below is a sample set of parameters for launching MegaMolBART finetuning for a retrosynthesis predition 
...
# located in examples/molecule/megamolbart/conf/downstream_retro_uspto50k.yaml

BIONEMO_IMAGE="??" # BioNeMo container image
WANDB_API_KEY=?? # Add your WANDB API KEY

CONFIG_NAME='downstream_retro_uspto50k' # name of the yaml config file with parameters 
...
MICRO_BATCH_SIZE=256 # micro batch size per GPU
...
# Logging
# =========================
PROJECT_NAME="megamolbart_downstream_retro"  # project name, will be used for logging
EXP_TAG="" # any additional experiment info, can be empty
EXP_NAME="mmb_retro_batch${MICRO_BATCH_SIZE}_gradacc${ACCUMULATE_GRAD_BATCHES}_nodes${SLURM_JOB_NUM_NODES}${EXP_TAG}"
...
# Mounts
# =========================
DATA_PATH="??" # Directory containing USPTO50k data
...
&& python /opt/nvidia/bionemo/examples/molecule/megamolbart/downstream_retro.py \
