...
### CONFIG ###
CONFIG_FILE=pretrain_xsmall_span_aug_mlp_valid
PROJECT=MegaMolBART
...
RUN_SCRIPT="pretrain_mlp_validation.py"
RUN_SCRIPT_DIRECTORY="${PROJECT_MOUNT}/examples/molecule/megamolbart"

usage() {
cat <<EOF
USAGE: pretrain_quick_mlp_validation.sh
MegaMolBART pretrain script with MLP validation in the loop
----------------------------------------
pretrain_quick_mlp_validation.sh [command]
    valid commands:
        preprocess
        train
