...
def get_tokenizer(model_path):
    tokenizer = SentencePieceTokenizerAdapter(
        SentencePieceTokenizer(model_path=model_path,
                                       special_tokens=None, legacy=False)
    )
    return tokenizer

@pytest.mark.parametrize(
    "model_path", get_tokenizer_model_paths(),
)
...
@pytest.mark.parametrize("test_input,n_calls,model_path", [
...
def test_dataloader_collate(model_path):
    tokenizer = get_tokenizer(model_path)
    dataloader = ProteinBertCollate(tokenizer=tokenizer,
                                  seq_length=50,
                                  pad_size_divisible_by_8=True)
    ...
    for idx, loss_mask in enumerate(collated_output["loss_mask"].numpy()):
        num_ones = 0
        for m in loss_mask:
            if m==1: num_ones = num_ones + 1
        assert num_ones == num_mods_total[idx], "Loss mask not accurate"
    assert torch.equal(collated_output["padding_mask"], torch.tensor(padding_mask)) == True, "Padding mask mismatch"
    assert torch.equal(collated_output["labels"], torch.tensor(padded_tokens)) == True, "Labels mismatch"