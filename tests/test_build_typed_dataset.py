...
def test_dataset_builder_fasta_fields():
    ...
    dataset = build_typed_dataset(dataset_paths=filepath, data_impl=data_impl, use_upsampling=False, cfg=cfg,
                                  num_samples=None)

    assert isinstance(dataset, _DATA_IMPL_TYPE_CLS[data_impl]), f'Dataset with data_imp={data_impl} should be ' \
                                                                f'of class {_DATA_IMPL_TYPE_CLS[data_impl].__name__}'

    assert len(dataset) == 100
    assert [k for k in dataset[57].keys()] == ['n', 'Tax', 'TaxID', 'RepID', 'id', 'sequence']
    ...
    assert (set(seq) - {'Y', 'N', 'W', 'E', 'F', 'L', 'A', 'K', 'R', 'D',
                        'C', 'Q', 'S', 'H', 'I', 'V', 'M', 'G', 'P', 'T'}) == set()
...
def test_dataset_builder_csv_mmap():
    ...
    kwargs = {'newline_int': 10, 'header_lines': 1, 'workers': 1, 'sort_dataset_paths': True, 'data_sep': ',',
              'data_col': 1}
...
def test_dataset_builder_csv_fields_mmap():
    ...
    kwargs = {'newline_int': 10, 'header_lines': 1, 'workers': 1, 'sort_dataset_paths': False,
              'data_sep': ',', 'data_fields': {'products': 3, 'reactants': 2}}
    ...
    assert isinstance(dataset, _DATA_IMPL_TYPE_CLS[data_impl]), f'Dataset with data_imp={data_impl} should be ' \
                                                                f'of class {_DATA_IMPL_TYPE_CLS[data_impl].__name__}'
    assert dataset[0] == {'products': 'COC(=O)CCC(=O)c1ccc(OC2CCCCO2)cc1O',
    ...
    dataset_2 = build_typed_dataset(dataset_paths=data_paths, data_impl=data_impl, use_upsampling=False, cfg=cfg,
                                    num_samples=None)
...
def test_dataset_builder_upsampling():
    ...
    num_samples = 1000
    dataset = build_typed_dataset(dataset_paths=filepath, data_impl=data_impl, use_upsampling=True, cfg=cfg,
                                  num_samples=num_samples)
    assert len(dataset) == num_samples and len(dataset._dataset) == 10

    assert isinstance(dataset, NeMoUpsampling) and isinstance(dataset._dataset,
                                                              _DATA_IMPL_TYPE_CLS[_CSV_FIELDS_MMAP_TYPE])