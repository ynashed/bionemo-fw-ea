...
class SingleCellRowDatasetCore(ABC):
    ...
    def shape(self) -> Tuple[int, List[int]]:
        """
        Returns the shape of the object, which may be ragged. A ragged dataset
        is where the number and dimension of features can be different at every
        row.
        """
        raise NotImplementedError()
    ...
    def sparsity(self) -> float:
        ...
        If there are no values, the sparsity is defined as 0.0.
        """
        total_values = self.number_of_values()
        if total_values == 0:
            return 0.0  # Avoid division by zero; define sparsity as 0.0 when there are no values
