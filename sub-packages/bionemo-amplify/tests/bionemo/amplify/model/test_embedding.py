...
from bionemo.amplify.data.tokenizer import BioNeMoAutoTokenizer, get_tokenizer
from bionemo.amplify.model.embedding import AMPLIFY_MASK_RATIO_TRAIN, AMPLIFYEmbedding
...
@pytest.fixture(scope="module")
def embedding(tokenizer) -> AMPLIFYEmbedding:
    with megatron_parallel_state_utils.distributed_model_parallel_state():
        config = AMPLIFYConfig(seq_length=20, hidden_size=128)
        model = config.configure_model(tokenizer)
        yield model.embedding


def test_init(embedding, tokenizer):
    assert isinstance(embedding, AMPLIFYEmbedding)
...
def test_apply_amplify_customization(embedding):
    ...
    # Call the _apply_amplify_customization function
    output_embeddings, embeddings_mask = embedding._apply_amplify_customization(
