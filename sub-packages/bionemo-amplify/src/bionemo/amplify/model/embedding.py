...
__all__: Sequence[str] = (
    "AMPLIFYEmbedding",
    "AMPLIFY_MASK_RATIO_TRAIN",
)

AMPLIFY_MASK_RATIO_TRAIN = 0.15 * 0.8  # Hardcoded as the ratio used in all ESM model training runs


class AMPLIFYEmbedding(LanguageModelEmbedding):
    """AMPLIFY Embedding with custom logic for attention masking and token dropout."""
    ...
    def __init__(
        ...
        """Initialize the AMPLIFY Embedding module."""
        super().__init__(
            config=config,
    ...
    def _apply_amplify_customization(
        self, word_embeddings: Tensor, input_ids: Tensor, attention_mask: Tensor
    ) -> Tuple[Tensor, Tensor]:
        """AMPLIFY customization for attention masking and token dropout.
        ...
        if embeddings_mask is not None and self.token_dropout:
            ...
            scale_factor = (1 - AMPLIFY_MASK_RATIO_TRAIN) / (1 - mask_ratio_observed)[:, None, None]
    ...
    def forward(
        ...
        word_embeddings, embeddings_mask = self._apply_amplify_customization(word_embeddings, input_ids, attention_mask)
        ...
        # AMPLIFY Customization: include attention masking from AMPLIFY
        if embeddings_mask is not None and self.use_attention_mask:
