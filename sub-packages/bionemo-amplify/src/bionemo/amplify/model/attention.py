...
__all__: Sequence[str] = ("AMPLIFYDotProductAttention",)


class AMPLIFYDotProductAttention(DotProductAttention):
    """AMPLIFY-Specific core attention.
    ...
    def forward(
        ...
        """Forward pass of the AMPLIFYDotProductAttention module.
        ...
        if self.config.use_esm_attention:
            ...
            # attention mask in AMPLIFY. The multiplication by -3.4028e+38 (float32 min_val) is
            # similarly motivated by AMPLIFY's masking approach, which forces softmax of attention scores
            ...
            attention_probs: Tensor = self.amplify_scale_mask_softmax(
                attention_scores.masked_fill(attention_mask[:, :, 0:1, :].to(bool), min_val)
            )
        # END AMPLIFY Customization
        else:
    ...
    def amplify_scale_mask_softmax(
        self,
        input: Tensor,
